...
Climatelabs %>% filter(Profile %in% c(labs[1], labs[12:14])) %>% 
  ...
  scale_color_manual(values=c('#F8766D','#7CAE00', '#00BFC4', 'black')) +
   theme(
...
A1 <- read_csv("Tables/Appendix_A.csv")

A1 %>% flextable() %>% theme_booktabs(x, bold_header = TRUE)  %>% 
  set_caption("Operationalization of the framework") %>%
#  bold(i = 1:29, j = 1, bold = TRUE, part = "body") %>% 
#  hline(i = c(4,12,17,20), j = 1:4, part="body") %>% 
#  fontsize(size = 8, part = "all") %>% 
...
```{r, fig.height=12, fig.width=6, out.width='50%'}
# ------- figure- ---
fig3 <- readxl::read_excel("data/Data-competences-onlyradar.xlsx")
#names(fig3)
labs <- fig3 %>% pull(Profile) 
...
Climatelabs <- 
  fig3 %>% filter(Profile %in% labs[5:14]) %>% 
  mutate(Country = c(rep("Colombia",5), rep("Brazil",3), rep("Mexico",2))) %>% 
  select(Country, Profile, Medi.1:Eval.1) %>% 
  pivot_longer(cols = Medi.1:Eval.1 )
  

Climatelabs_bg <- fig3 %>% filter(Profile %in% labs[5:14]) %>% 
  mutate(Country = c(rep("Colombia",5), rep("Brazil",3), rep("Mexico",2))) %>% 
  select(Country, Profile, Medi.1:Eval.1) %>% 
  pivot_longer(cols = Medi.1:Eval.1 ) %>% 
