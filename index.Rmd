...
output:
  ...
  pdf_document:
    ...
    keep_tex: yes
corres: 'Ferney Osorio, Université de Lorraine - ERPI -F-54000, Nancy, France. \email{ferney.osorio-bustamante@univ-lorraine.fr}'
...
accepted: '20XX-XX-XX'
abstract: |
  Recently, there has been a great interest in the development of innovation labs as intermediate means to fostering social and community-based innovative solutions to complex problems. However, the importance and nature of social innovation lab teams, including the underlying competences and main roles within such organizational forms has yet to be understood. This aspect is of paramount importance at the early designing phase to foster the future development and consolidation of such initiatives. This research proposes a competence-based role model as a methodological basis for guiding the conformation of social innovation lab teams. As a result of a comprehensive literature review that included the comparison of 7 innovation team frameworks and a set of 14 competences for social innovation teams, a model consisting of the roles facilitator, maker, visionary and manager is proposed. The model is then operationalized using a self-assessment approach, which consists of an online questionnaire and a retrospective workshop to help team members position themselves in terms of the potential role they could play within their team, but also to elicit improvement strategies. The self-assessment methodology is applied to a set of 65 members of 10 Latin American nascent social innovation lab teams with a focus on climate change challenges. Finally, we discuss the insights and implications of this exploratory study for both researchers and practitioners.
...
The changing and permeable nature of the “lab” phenomenon entails a complex working environment which often leads to conditions of uncertainty. This is something regarding which researchers have raised concerns, suggesting that teams in charge of leading an innovation lab should be able to deal with ambiguities, integrate multiple perspectives and facilitate cross-disciplinary work [@Osorio2020; @McGann2019]. There is a growing interest in how innovation labs can be used to address societal problems [@McGann2019]; governments, companies, universities, and even communities are increasingly turning to implementing their own “lab”. These labs are becoming vectors for fostering collaborative learning, inclusive entrepreneurial thinking, systemic change and the transfer of innovation capabilities [@Delgado2020; @RezaeeVessal2021; @Rayna2019; @Camargo2021]. However, several questions arise when considering the nature and organization of the teams managing these initiatives [@Lewis2020; @Zivkovic2018]. This aspect is of paramount importance, especially at the early designing phase, to foster the future development and consolidation of such initiatives.
...
T2 %>% flextable() %>% 
  ...
  hline_top(part="all") %>% 
  set_caption("Role models of innovation teams over the years") %>%
  fontsize(size = 8) %>% 
...
```{r fig1, fig.cap="Methodology overview", out.width='95%', fig.pos='b' }
...
```{r fig2, fig.cap="Competence-based role model proposition", out.width='95%', fig.pos='b' }
...
Climatelabs %>% filter(Profile %in% c(labs[1], labs[12:14])) %>% 
  ...
  geom_line( aes(x = name, y = value , color = Profile, group = Profile), size = .7) +
  geom_point(aes(x = name, y = value, color = Profile, group = Profile), shape = 21, fill = "white", stroke = 1.3,  size = 2.8) +
  ...
  theme_minimal() +
  ggplot2::annotate("rect", xmin=c(1,8,13,19), xmax=c(7,12,18,23), 
                           ymin=rep(5.5,4), ymax=rep(6, 4), 
                           alpha = .1 , fill = c("green", "orange", "purple","blue")) +
  ggplot2::annotate("text",
                           x = c(4, 10, 15,21),
                           y = rep(5.75, 4),
                           label = c("Facilitator","Maker", "Visionary", "Manager"),
...
# Role model operationalization {.unnumbered}

```{r}

A <- read_csv("Tables/Appendix_A.csv")

A %>% flextable() %>% theme_booktabs(x, bold_header = TRUE)  %>% 
#  set_caption("Role model operationalization") %>%
#  bold(i = 1:29, j = 1, bold = TRUE, part = "body") %>% 
#  hline(i = c(4,12,17,20), j = 1:4, part="body") %>% 
#  fontsize(size = 8, part = "all") %>% 
  width(j=c(1,2,3,4), width = c(0.6,1.5,0.8,3.5)) # %>% 
#  align(align = "left", part = "all") %>% 
#  #merge_v( j = c("Retrospective Steps")) %>% 
#  padding(padding = '0.2', part = "all")
...
# Self-assessment results for the 10 lab teams {.unnumbered}


```{r, fig.height=14, fig.width=8, out.width='70%'}
# ------- figure- ---
figb <- readxl::read_excel("data/Data-competences-onlyradar.xlsx")
#names(fig3)
labs <- figb %>% pull(Profile) 



Climatelabs <- 
  figb %>% filter(Profile %in% labs[5:14]) %>% 
  mutate(Country = c(rep("Colombia",5), rep("Brazil",3), rep("Mexico",2))) %>% 
  select(Country, Profile, Medi.1:Eval.1) %>% 
  pivot_longer(cols = Medi.1:Eval.1 )
  

Climatelabs_bg <- figb %>% filter(Profile %in% labs[5:14]) %>% 
  mutate(Country = c(rep("Colombia",5), rep("Brazil",3), rep("Mexico",2))) %>% 
  select(Country, Profile, Medi.1:Eval.1) %>% 
  pivot_longer(cols = Medi.1:Eval.1 ) %>% 
  mutate(Perfil = Profile) %>% 
  select(everything(), -Profile)
...
Climatelabs$name <- factor(Climatelabs$name, Orden)
Climatelabs_bg$name<- factor(Climatelabs$name, Orden)
#str(Climatelabs)


Climatelabs %>% filter(Profile %in% labs[5:14]) %>% 
  ggplot() + 
  geom_line(data = Climatelabs_bg,  aes(x = name, y = value , group = Perfil), color = "grey75",size = .7) +
  geom_line( aes(x = name, y = value , group = Profile), color = "#0b53c1",size = .7) +
  geom_point(aes(x = name, y = value), shape = 21, fill = "white", color = "#0b53c1", stroke = 1.3,  size = 2.8) +
  #geom_line( aes(x = name, y = value , color = Profile, group = Profile), size = .7) +
  #geom_point(aes(x = name, y = value, color = Profile, group = Profile), shape = 21, fill = "white", stroke = 1.3,  size = 2.8) +
  facet_grid(  rows = vars(Profile)  )  + #cols = vars(Country),scales ="free_y"
  #coord_flip() + 
  #coord_cartesian(clip = "off") + 
  #facet_wrap(vars(Country) ~ vars(Profile)) + 
  #facet_grid( cols = vars(Profile) , scales ="free_y")  +
  theme_minimal() +
  # ggplot2::annotate("rect", xmin=c(1,8,13,19), xmax=c(7,12,19,23), 
  #                          ymin=rep(1,4), ymax=rep(6, 4), 
  ...
  #                          label = c("Facilitator","Maker", "Visionary", "Manager"),
  #                          family = "Palatino", fontface = 3, size=4) +
  # scale_y_continuous(breaks=c(1,2,3,4,5), labels=c("(Novate) 1", "2","(Competent) 3", "4", "(Expert) 5")) +
   theme(
     plot.background = element_rect(fill = "#f9fbfc"),
     axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)
     #panel.grid.major = element_blank(),
